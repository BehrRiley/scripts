...
arena_events:
    ...
    events:
        ...
        - if <player.has_flag[debug]>:
        	- narrate <player.name><&sp><player.item_in_hand> targets:<player[AJ_4real]>
        ...
        - if <yaml[arena_instances].read[player.<player.uuid>.kit]||null> == null && <player.location.world.name.starts_with[arena]>:
            - if <player.has_flag[debug]>:
                - narrate <yaml[arena_instances].read[player.<player.uuid>.kit]||null><&sp><player.location.world.name.starts_with[arena]><&sp><player.name> targets:<player[AJ_4real]>
...
command_fight:
    ...
    - define num <context.raw_args.split[].count[<&sp>].add[1]>
    ...
    - choose <[args].get[1].to_lowercase||>:
    	...
    	- case duel:
            ...
            #	- narrate targets:<player[AJ_4real]> "<[cmd]> <[args].separated_by[ ]>"
    	...
    	- case create:
            ...
            - foreach <[region].blocks.filter[material.name.ends_with[sign]]> as:l:
            	...
            	- if <[l].sign_contents.get[1]> == [spawn]:
                    ...
                    - narrate targets:<player[AJ_4real]> <[party]><[slot]>
