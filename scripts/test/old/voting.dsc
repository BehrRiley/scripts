...
top_voters_gui:
    ...
    procedural items:
    #- define voters <server.players_flagged[votes.service].sort_by_number[flag[votes.service].values.sum]||<list[]>>
...
proc_most_used_website:
    ...
    - foreach <server.players> as:p:
        - if <[p].has_flag[votes.service]>:
            - foreach <[p].flag[votes.service].keys> as:k:
                - define websites <[websites].with[<[k]>].as[<[p].flag[votes.service.<[k]>].add[<[websites].get[<[k]>]>]>]>
...
proc_most_used_website_this_month:
    ...
    - define websites <map[]>
    - foreach <server.players> as:p:
        - if <[p].has_flag[votes.month.<util.time_now.start_of_month.epoch_millis.div[1000]>.service]>:
            - foreach <[p].flag[votes.month.<util.time_now.start_of_month.epoch_millis.div[1000]>.service].keys> as:k:
                - define websites <[websites].with[<[k]>].as[<[p].flag[votes.month.<util.time_now.start_of_month.epoch_millis.div[1000]>.service.<[k]>].add[<[websites].get[<[k]>]>]>]>
    - define highest <[websites].sort_by_value>
    - determine <[highest].keys.last||Unknown>
        ...
        - foreach <yaml[voting_rewards].list_keys[rewards.tier.<[tier]>.every]> as:k:
            - if <[total_votes].mod[<[k]>]> == 0:
                ...
                - foreach <yaml[voting_rewards].read[rewards.tier.<[tier]>.every.<[k]>.commands]> as:command:
                    - execute as_server <[command].replace[<&pc>player_name<&pc>].with[<[player].name>]>
        - flag server votes:+:1
            ...
            - if <server.flag[votes].mod[<[k]>]> == 0:
                - foreach <server.online_players.filter[has_flag[voted]]> as:pl:
                    - foreach <yaml[voting_rewards].read[rewards.party.every.<[k]>.commands.players_who_voted]> as:cmd:
                        - execute as_server <[cmd].replace[<&pc>player_name<&pc>].with[<[pl].name>]>
